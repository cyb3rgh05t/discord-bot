...
class PlexTicketSetup(BaseTicketSetup):
    """Cog for setting up the Plex ticket system"""
    ...
    @app_commands.command(
        name="plexticketsetup", description="Setup your Plex ticket panel"
    )
    ...
    async def plexticketsetup(
        ...
        try:
            ...
            # Create embed
            embed = discord.Embed(
                title=f"{guild.name} | Plex Ticket System",
    ...
    async def cog_load(self):
        ...
        self.bot.tree.add_command(self.plexticketsetup, guild=guild)


async def setup(bot):
    await bot.add_cog(PlexTicketSetup(bot))
    logger.debug("PlexTicketSetup cog loaded.")
